...
int CMesh::GetClosestNode(XYZ Position) const
{
	...
	for (itNode = m_Nodes.begin(), i=0; itNode != m_Nodes.end(); ++itNode, ++i)
	{
		...
		//if (itNode == m_Nodes.begin() || dDistSqrd < dClosestDistSqrd)
		{
...
int CMesh::GetClosestNodeDistance(XYZ Position, double ClosestDist ) const
{
	vector<XYZ>::const_iterator itNode;
	double dClosestDistSqrd = -1;
	double dDistSqrd;
	...
	for (itNode = m_Nodes.begin(), i=0; itNode != m_Nodes.end(); ++itNode, ++i)
	{
		...
		{
			dClosestDistSqrd = dDistSqrd;
			if ( dClosestDistSqrd <= ClosestDist )
				iClosestNode = i;
		}
	}
	return iClosestNode;
