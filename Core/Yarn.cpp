...
bool CYarn::PointInsideYarn(XYZ Point, XYZ *pTangent, XY *pLoc, double* pVolumeFraction, double* pDistanceToSurface, double dTolerance, XYZ *pOrientation, XYZ *pUp) const
{
	//PROFILE_FUNC()
	if (!BuildYarnIfNeeded(SURFACE))
		return false;
	...
	for (i=0; i<iNumSegments; ++i)
	{
		...
		N1 = m_pInterpolation->GetNode(m_MasterNodes, i, 0);
		N2 = m_pInterpolation->GetNode(m_MasterNodes, i, 1);
